...
class electronice(object):
    def __init__(self,  inp, n_in, A,D,Kx,Ky,inverse=False):
        ...
        if inverse == False:
            ...
            self.det = T.log(abs(T.prod(T.tanh(self.A))))+T.sum(T.log(abs(self.D)))+T.mean(T.log(T.prod(self.activD,axis=0))) # Calculate the contribution of the layer towards the log-det term
        else:
            ...
            self.det = T.log(abs(T.prod(T.tanh(self.A)))*T.prod(self.D))+T.mean(T.log(T.prod(self.activD,axis=0)))
    ...
    def knot_activ(self,x):
        ...
        Else
             y = (x-1)*(tanh(Ky) - 1)/(tanh(Kx) - 1) + 1
        '''
        return T.lt(x,T.tanh(self.Kx))*( (x+1)*(T.tanh(self.Ky)+1)/(T.tanh(self.Kx)+1) - 1 )  + T.ge(x,T.tanh(self.Kx)) *( (x-1)*(T.tanh(self.Ky)-1)/(T.tanh(self.Kx)-1) + 1 )
    ...
    def knot_activ_inverse(self,x):
        ...
        '''
        return T.lt(x,T.tanh(self.Ky))*( (x+1)*(T.tanh(self.Kx)+1)/(T.tanh(self.Ky)+1) - 1 )  + T.ge(x,T.tanh(self.Ky)) *( (x-1)*(T.tanh(self.Kx)-1)/(T.tanh(self.Ky)-1) + 1 )
    
    def knot_Det(self,x):
        '''
        Calculate the Jacobian of the piece-wise linear activation
        '''
        return T.lt(x,T.tanh(self.Kx))*( (T.tanh(self.Ky)+1)/(T.tanh(self.Kx)+1)  )  + T.ge(x,T.tanh(self.Kx)) *( (T.tanh(self.Ky)-1)/(T.tanh(self.Kx)-1)  )

    def knot_Det_inverse(self,x):
        '''
        Returns the Jacobian of the piece-wise linear activation for the inverse transformation
        '''
        return T.lt(x,T.tanh(self.Ky))*( (T.tanh(self.Kx)+1)/(T.tanh(self.Ky)+1)  )  + T.ge(x,T.tanh(self.Ky)) *((T.tanh(self.Kx)-1)/(T.tanh(self.Ky)-1)  )
...
class Params1(object):
    ...
    def __init__(self,n_in,A=None,D=None,Kx=None,Ky=None,Db=None):
        ...
        if Kx is None:
            ...
            Kx = np.random.normal(0.5,0.1,(n_in,))
        ...
        if Ky is None:
            ...
            Ky = np.random.normal(0.5,0.1,(n_in,))

            
        Kx = theano.shared(Kx,borrow=True)
        Ky = theano.shared(Ky,borrow=True)
